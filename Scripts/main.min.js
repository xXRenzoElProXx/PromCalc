        ...
        var e = function () {
            try {
                ...
                for (e = 0; e < t.length; e++) {
                    ...
                    // Cada fila lleva su propia clase de entrada (row-enter) y un
                    // delay creciente vía variable CSS --row-i para el stagger
                    // definido en styles.css (@keyframes rowEnter).
                    a += "<tr class='" + rowClass + " row-enter' style='--row-i: " + e + "'>" +
                ...
                // Aparición del bloque de resultados: en vez de saltar de
                // display:none a block (sin transición posible para eso en
                // CSS puro), se quita y se vuelve a poner la clase que
                // dispara la animación de fade+slide (@keyframes resultIn en
                // styles.css), incluso si #result ya estaba visible por una
                // evaluación previa.
                var $result = $("#result");
                $result.css({ display: "block" });
                $result.removeClass("result-enter");
                void $result[0].offsetWidth;
                $result.addClass("result-enter");
            } catch (e) {
                err(e);
            }
        };
...
// Pequeño "tick" tipo pantalla digital: aplica una clase momentánea que
// dispara un flicker de opacidad/brillo (@keyframes lcdTick en styles.css)
// cada vez que un span de resultado cambia de valor.
var tickElement = function (selector) {
    var el = document.querySelector(selector);
    if (!el) return;
    el.classList.remove("lcd-tick");
    void el.offsetWidth;
    el.classList.add("lcd-tick");
};
